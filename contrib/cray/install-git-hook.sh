...
then
	...
	case "${branchname}" in
		...
		v*.x	)
			errval=1
			printf '\n%s: ERROR:  commits to "%s" are disallowed\n\n'\
...
then
	gittopdir=$(realpath -e $(git rev-parse --show-toplevel))
	if [ ${?} -ne 0 ]
	then
		errval=1	 
		printf '\n%s: ERROR:  failed to find git working top dir\n\n'\
			 "${scriptname}"
	else		
		git diff --no-ext-diff --cached \
		| ${gittopdir}/contrib/cray/checkpatch.pl\
			--no-tree\
			--no-signoff\
			--ignore=AVOID_EXTERNS,FILE_PATH_CHANGES,SSCANF_TO_KSTRTO
		if [ ${?} -ne 0 ]
		then
			errval=1
		fi
